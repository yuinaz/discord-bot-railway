...
def ensure_globals(s: str) -> str:
    ...
    m = re.search(r"^(?:from\s+\S+\s+import\s+.*|import\s+\S+).*$", s, re.M)
    ...
    if m:
        ...
        while True:
            ...
            nxt = re.search(r"^(?:from\s+\S+\s+import\s+.*|import\s+\S+).*$", s[end:], re.M)
...
def insert_guard_once(s: str) -> str:
    ...
    for mlog in re.finditer(re.escape(LOG_NEEDLE), s):
        ...
        defs = list(re.finditer(r"^([ \t]*)async\s+def\s+\w+\s*\([^)]*\)\s*:\s*$", s[: mlog.start()], re.M))
        ...
        next_def = re.search(r"^[ \t]*async\s+def\s+\w+\s*\(|^[ \t]*class\s+\w+\s*\(", s[header_end:], re.M)
