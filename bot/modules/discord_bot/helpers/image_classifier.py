...
import os, io, json, math, time, requests
...
def _cos(a: List[float], b: List[float]) -> float:
    ...
    import numpy as np
    va = np.array(a, dtype="float32"); vb = np.array(b, dtype="float32")
...
def classify_image(image_bytes: bytes):
    ...
    try:
        ...
        return {"enabled": True, "score": s, "threshold": IC_THRESHOLD, "verdict": verdict, "action": IC_ACTION}
