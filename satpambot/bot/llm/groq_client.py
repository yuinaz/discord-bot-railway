...
async def groq_chat(messages: list[dict], *, model: str | None = None, temperature: float = 0.6, max_tokens: int | None = 800) -> str:
    model = model or _MODEL
    resp = await _call_chat(messages, model, stream=False, max_completion_tokens=max_tokens, temperature=temperature)
    return resp.choices[0].message.content or ""

async def groq_stream(messages: list[dict], *, model: str | None = None, temperature: float = 0.6, max_tokens: int | None = 800) -> t.AsyncIterator[str]:
    model = model or _MODEL
    stream = await _call_chat(messages, model, stream=True, max_completion_tokens=max_tokens, temperature=temperature)
