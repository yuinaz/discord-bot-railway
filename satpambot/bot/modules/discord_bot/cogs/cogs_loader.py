...
async def _safe_load(bot: commands.Bot, name: str) -> None:
    try:
        ...
        if isinstance(loaded, dict) and name in loaded: log.info("[cogs_loader] already loaded: %s", name); return
    except Exception: pass
    if name in sys.modules: log.info("[cogs_loader] already in sys.modules: %s", name); return
    try:
        await bot.load_extension(name); log.info("[cogs_loader] loaded: %s", name)
    except Exception as e:
        msg = f"{e}"
        if "ExtensionAlreadyLoaded" in msg or "already loaded" in msg:
            log.info("[cogs_loader] already loaded (caught generic): %s", name)
        else:
            log.warning("[cogs_loader] failed to load %s: %r", name, e)
async def setup(bot: commands.Bot) -> None:
    for ext in EXTENSIONS: await _safe_load(bot, ext)
    try: log.info("[cogs_loader] summary loaded: %s", sorted(getattr(bot, "extensions", {}).keys()))
    except Exception: pass
