# -*- coding: utf-8 -*-
"""
a06_embed_scribe_post_shim_v3 (fixed)
...
def _inject():
    try:
        ...
        log.warning("[post_shim_v3] embed_scribe module missing: %r", e); return False
    ...
    if ES is None:
        ...
        log.info("[post_shim_v3] provided fallback EmbedScribe with async upsert()")
        return True
    if not hasattr(ES, "upsert"):
        async def upsert(self, channel, key, embed, pin=False): return None
        setattr(ES, "upsert", upsert)
        log.info("[post_shim_v3] added missing upsert() to EmbedScribe")
        return True
    ...
    if not inspect.iscoroutinefunction(up):
        ...
        log.info("[post_shim_v3] wrapped non-async upsert() with async shim")
    return True

# eager inject to satisfy offline/smoke import
try: _inject()
except Exception as _e: log.debug("[post_shim_v3] eager inject failed: %r", _e)

class PostShimV3(commands.Cog):
    def __init__(self, bot): self.bot=bot
    @commands.Cog.listener()
    async def on_ready(self): _inject()

async def setup_async(bot): await bot.add_cog(PostShimV3(bot))
