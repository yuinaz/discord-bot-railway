...
LOCK_DIR = Path("data/sticky"); LOCK_DIR.mkdir(parents=True, exist_ok=True)
LOCK_FILE = LOCK_DIR / "leader.lock"
LOCK_STALE_SEC = int(os.getenv("STICKY_LOCK_STALE_SEC", "180"))
LOCK_RENEW_SEC = int(os.getenv("STICKY_LOCK_RENEW_SEC", "30"))
...
def _format_uptime(start):
    ...
    sec = int(delta.total_seconds()); m, s = divmod(sec, 60); h, m = divmod(m, 60); d, h = divmod(h, 24)
    parts = []
...
def _load_store():
    try:
        if STORE_PATH.exists(): return json.loads(STORE_PATH.read_text("utf-8"))
...
class StickyStatus(commands.Cog):
    def __init__(self, bot: commands.Bot):
        ...
        self.status_ids = self._parse_ids(os.getenv("STICKY_STATUS_CHANNEL_IDS", os.getenv("STICKY_CHANNEL_ID","")))
        self.latency_ids = self._parse_ids(os.getenv("STICKY_LATENCY_CHANNEL_IDS", os.getenv("STICKY_CHANNEL_ID","")))
    ...
    async def on_ready(self):
        ...
        if self._leader:
            ...
            if not self.latency_loop.is_running(): self.latency_loop.start()

    async def _resolve_default_channels(self):
        if self.status_ids and self.latency_ids: return
        for g in self.bot.guilds:
            me = g.me
            for ch in g.text_channels:
                try:
                    if ch.name.lower() == self.pref_name and ch.permissions_for(me).send_messages:
                        self.status_ids.add(ch.id); self.latency_ids.add(ch.id); return
                except Exception: continue

    async def _elect_leader(self):
        if os.getenv("STICKY_NO_LOCK"): self._leader = True; return
        pid = os.getpid()
        while True:
            try:
                if not LOCK_FILE.exists():
                    LOCK_FILE.write_text(json.dumps({"pid": pid, "ts": _now_utc().isoformat()}), encoding="utf-8")
                    self._leader = True; break
                else:
    ...
    async def _leader_heartbeat(self):
        pid = os.getpid()
        while True:
            try: LOCK_FILE.write_text(json.dumps({"pid": pid, "ts": _now_utc().isoformat()}), encoding="utf-8")
            except Exception: pass
    ...
    def _presence_text(self):
        for g in self.bot.guilds:
            me = g.me
            if me and getattr(me, "status", None) is not None: return str(me.status)
        return "online"

    def _status_payload(self):
        u = self.bot.user
        name = f"{u.name}#{u.discriminator}" if u and getattr(u, "discriminator", "0") != "0" else (u.name if u else "Bot")
        ...
        e = discord.Embed(title="SatpamBot Status", description="Status ringkas bot.", color=0x30D158, timestamp=datetime.now(TZ_WIB))
    ...
    async def _ensure_messages(self):
        for cid in list(self.status_ids): await self._ensure_single(cid, "status")
        for cid in list(self.latency_ids): await self._ensure_single(cid, "latency")
    ...
    async def _scan_and_claim(self, ch: discord.TextChannel, key: str):
        ...
        async for m in ch.history(limit=50):
            ...
            if not is_ours and m.content:
                if key=="latency" and "Latency" in m.content and "SatpamBot online" in m.content: is_ours = True
                if key=="status" and m.content.strip().startswith("✅ Online"): is_ours = True
            if is_ours:
    ...
    async def _ensure_single(self, channel_id: int, key: str):
        ...
        msg_id = rec.get("message_id"); msg = None
        if msg_id:
        ...
        async with self._lock:
            ...
            if last_ts:
                try:
                    ...
                    if age < min_periodic and not should: return
                except: pass
            try:
                await msg.edit(content=new_content, embed=new_embed)
                rec["last_content"] = new_content; rec["last_ts"] = _now_utc().isoformat()
                self.store[key][dkey] = rec; _save_store(self.store)
            except Exception: pass
