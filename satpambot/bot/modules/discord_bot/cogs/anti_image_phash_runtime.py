...
EXEMPT_CHANNELS = set(
    ...
    [c.strip().lower() for c in (os.getenv("PHISH_EXEMPT_CHANNELS", "") or "").split(",") if c.strip()]
...
class AntiImagePhashRuntime(commands.Cog):
    ...
    - Startup: muat DB dari pesan marker
    ...
    lalu BACKFILL: scan riwayat gambar di inbox → hitung pHash → append ke DB.
    ...
    async def on_ready(self):
        ...
        await self._log(
            ...
            f"🔐 Runtime pHash DB loaded: {len(self.db_hashes)} entries (thresh={HAMMING_THRESH}, autoban={'ON' if AUTOBAN else 'OFF'})",  # noqa: E501
                ...
                for t in threads:
                    ...
                    if hashes:
                        ...
                        try:
                            ...
                            gid = int(
                                ...
                                getattr(
                                    ...
                                    getattr(getattr(locals().get("msg", None), "guild", None), "id", 0),
                        ...
                        await self._log(
                            ...
                            f"♻️ pHash DB loaded from Discord: {len(hashes)} entries (src: #{getattr(msg.channel, 'name', '?')})",  # noqa: E501
                        ...
                        if PHASH_STORE_FILE:
                            ...
                            try:
                                ...
                                Path(PHASH_STORE_FILE).write_text(
                                    ...
                                    json.dumps({"phash": hashes}, ensure_ascii=False, indent=2),
    ...
    async def _append_hashes_to_db(self, thread: discord.Thread, new_hashes: List[str]):
        ...
        try:
            ...
            await self._log(
                ...
                f"🧩 Added {len(merged) - len(current)} pHash → DB now {len(merged)} entries.",
        ...
        try:
                    ...
                    try:
                        ...
                        if hashes:
                            ...
                            await self._log(
                                ...
                                message.guild if hasattr(message, "guild") else None,
                                ...
                                f"📥 pHash DB updated from message by {message.author} in #{getattr(message.channel, 'name', '?')} → {len(hashes)} entries",  # noqa: E501
            ...
            for att in imgs:
                ...
                if AUTOBAN and isinstance(message.guild, discord.Guild):
                    ...
                    try:
                        ...
                        await self._log(
                            ...
                            f"⚠️ Match (Hamming={d}) but failed to ban {message.author}: {e}",
                ...
                await self._log(
                    ...
                    f"✅ Enforced (Hamming={d} ≤ {HAMMING_THRESH}) on {message.author} in #{getattr(message.channel, 'name', '?')}",  # noqa: E501
