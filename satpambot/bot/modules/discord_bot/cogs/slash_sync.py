...
class SlashSync(commands.Cog):
    """Auto per-guild sync (≤5 guild) + tools. Moderator-friendly perms."""
        ...
        async with self._lock:
            ...
            try:
                ...
                if mode == "per-guild":
                    ...
                    log.info("[slash] global sync ok (auto mode)")
    ...
    async def on_guild_join(self, guild: discord.Guild):
        try:
            ...
            log.info("[slash] synced new guild: %s", guild.id)
        except Exception as e:
            log.exception("sync new guild failed: %s", e)

    # ===== Moderator-friendly commands =====
    ...
    async def sync_cmd(self, interaction: discord.Interaction, mode: str = "auto"):
        ...
        try:
            if mode == "global":
                await self.bot.tree.sync()
                return await interaction.followup.send("✅ Global sync OK.", ephemeral=True)
            if mode == "guild":
            ...
            # AUTO
            auto = self._decide_mode()
            return await self.sync_cmd(interaction, auto)
        except Exception as e:
            await interaction.followup.send(f"❌ Sync gagal: `{e}`", ephemeral=True)
    ...
    async def reload_cmd(self, interaction: discord.Interaction, module: str):
        ...
        candidates = [
            module,
            f"satpambot.bot.modules.discord_bot.cogs.{module}",
            f"bot.modules.discord_bot.cogs.{module}",
            f"modules.discord_bot.cogs.{module}",
        ]
        last = None
        for ext in candidates:
            try:
                await self.bot.reload_extension(ext)
                return await interaction.followup.send(f"🔁 Reload OK: `{ext}`", ephemeral=True)
            except Exception as e:
                last = e
