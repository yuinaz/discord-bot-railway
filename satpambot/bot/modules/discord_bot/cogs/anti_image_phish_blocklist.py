...
class AntiImagePhishBlocklist(commands.Cog):
    ...
    async def on_ready(self):
        ...
        await self._log(
            ...
            f"🔐 pHash blocklist loaded: {len(self.store.hashes)} entries "
    ...
    def _reload_blocklist(self) -> None:
        ...
        arr = _normalize_hashes_from_any(nobj) + [
            ...
            h for h in _normalize_hashes_from_any(lobj) if h not in set(_normalize_hashes_from_any(nobj))
        ...
        try:
            ...
            self.store.mt_new = self.store.path_new.stat().st_mtime
        ...
        try:
            ...
            self.store.mt_legacy = self.store.path_legacy.stat().st_mtime
                    ...
                    if self.autoban and isinstance(message.guild, discord.Guild):
                        ...
                        try:
                            ...
                            await self._log(
                                ...
                                f"🚫 Auto-banned {message.author} (Hamming={d} <= {self.threshold}) via pHash match.",
                            ...
                            await self._log(
                                ...
                                f"⚠️ Detected pHash match (Hamming={d}) but failed to ban {message.author}: {e}",
                        ...
                        await self._log(
                            ...
                            f"🧹 Deleted phishing image from {message.author} (Hamming={d} <= {self.threshold})",
