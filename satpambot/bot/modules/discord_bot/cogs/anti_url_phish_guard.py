        ...
        with open(WHITELIST_JSON, "r", encoding="utf-8") as f:
            ...
            if isinstance(obj, dict):
                ...
                if isinstance(arr, list):
                    ...
                    allow |= {_norm_domain(x) for x in arr}
...
class UrlPolicy:
    ...
    def merge_json(self, obj: Dict[str, Any]):
        ...
        if "autoban" in obj:
            ...
            self.autoban = bool(obj.get("autoban"))
        ...
        if "heur_autoban" in obj:
            ...
            self.heur_autoban = bool(obj.get("heur_autoban"))
        ...
        if "nsfw_autoban" in obj:
            ...
            self.nsfw_autoban = bool(obj.get("nsfw_autoban"))
...
class AntiUrlPhishGuard(commands.Cog):
    ...
    def _looks_suspicious(self, domains: List[str], content: str) -> bool:
        ...
        return any(p.search(" ".join(domains)) for p in SUSPICIOUS_PATTERNS) or any(
            ...
            p.search(content or "") for p in SUSPICIOUS_PATTERNS
    ...
    def _can_punish(self, m: discord.Message) -> bool:
        ...
        if perms and (perms.administrator or perms.manage_guild or perms.ban_members):
...
async def on_message(self, message: discord.Message):
    ...
    for code in invite_codes:
        ...
        if (
            ...
            self._is_whitelisted_url(full_url)
            ...
            or self._is_soft_exempt(message, content, full_url)
    ...
    if confirmed_nsfw and bool(policy.get("autoban_enabled", True)):
        ...
        await self._log(
            ...
            f"[nsfw-invite] BANNED {message.author} for https://discord.gg/{confirmed_code}",
    ...
    async def _post_ban_embed(
        ...
        self, message: discord.Message, reason: str, tag: str = "[auto-ban]", url: str | None = None
        ...
        """Kirim embed ban ke channel pelanggaran (WIB + alasan). Mirror Ban Log dimatikan — banlog_route menangani via on_member_ban."""  # noqa: E501
        ...
        category = (
            ...
            if "nsfw" in tag_l or "nsfw" in rsn_l
                ...
                if "invite" in tag_l or "unknown" in tag_l or (url and "discord.gg" in str(url).lower())
