...
class BasicBanCog(commands.Cog):
    ...
    @commands.command(
        ...
        help="Tempban: !tban @user <dur> [reason?]  contoh: !tban @user 7d spam",
    ...
    async def tban_cmd(self, ctx: commands.Context, member: discord.Member, duration: str, *, reason: str = ""):
        ...
        try:
            ...
            await ctx.reply(
                ...
                f"✅ Tempbanned {member.mention} selama {duration}" + (f" | alasan: {reason}" if reason else "")
            ...
            await self._log(
                ...
                f"[TBAN] {member} oleh {ctx.author} durasi {duration} | alasan: {reason or '-'}",
