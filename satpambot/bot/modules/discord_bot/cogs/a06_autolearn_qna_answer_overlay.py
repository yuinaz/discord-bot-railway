...
Auto-answer "Question by Leina" with "Answer by {Gemini|Groq}".
- Provider selection honors QNA_PROVIDER_ORDER
- Gemini detection now supports GEMINI_API_KEY (and common aliases)
"""
...
def _has_gemini() -> bool:
    # Support multiple env names for Gemini
...
async def _dedup_mark(mid: int) -> bool:
    try:
        ...
        if await cli.get_raw(key) is not None:
            return False
        await cli.setex(key, 60*60*24*30, "1")
        return True
    except Exception as e:
        log.debug("[qna-answer] dedup mark err: %r", e)
        return True
...
async def _call_llm(topic: str) -> Tuple[str, str]:
    try:
        ...
        return ("fallback", "Maaf, LLM tidak tersedia saat ini.")
    ...
    async def on_message(self, m: "discord.Message"):
        try:
            ...
            try:
                ...
                try:
                    await ch.send(embed=ans)
                except Exception as e2:
                    log.warning("[qna-answer] send failed: %r", e2)
