...
class AutoRoleAnywhere(commands.Cog):
    ...
    def _load_cfg(self):
        ...
        try:
            ...
            self.thread_name_map = {
                ...
                str(name): int(val["role"] if isinstance(val, dict) else int(val)) for name, val in tn.items()
    ...
    async def _ensure_baseline_persisted(self):
        ...
        marker_channel_id = int(self.cfg.get("state_marker_channel_id") or 0) or _env_int(
            ...
            "LOG_CHANNEL_ID", _env_int("LOG_CHANNEL_ID_RAW", 0)
    ...
    def _mapped_role_for(self, channel_id: int) -> Optional[int]:
        ...
        if channel_id in self.text_map:
            ...
            return self.text_map[channel_id]
        ...
        if channel_id in self.thread_map:
            ...
            return self.thread_map[channel_id]
    ...
    async def on_thread_member_join(self, member: discord.ThreadMember):
        ...
        if not bool(self.cfg.get("grant_on_thread_member_join", False)):
    ...
    async def _auto_prune_once(self):
        ...
        for g in list(self.bot.guilds):
            ...
            for cid, rid in self.text_map.items():
                ...
                if int(rid) in self.exempt_roles:
    ...
    async def status_cmd(self, itx: discord.Interaction):
        ...
        txt = f"auto_prune_enabled={self.cfg.get('auto_prune_enabled', True)} • interval={hrs}h • grace_remaining_hours={int(remain / 3600)}h"  # noqa: E501
    ...
    async def backfill_cmd(self, itx: discord.Interaction, limit: Optional[int] = None):
        ...
        lim = limit or int(self.cfg.get("backfill_default_limit", 800))
