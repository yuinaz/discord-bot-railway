...
class StatusPinUpdater(commands.Cog):
    def __init__(self, bot: commands.Bot):
        self.bot = bot; self._started_at = time.time()
        self._store: Dict[str, StatusRecord] = {}; self._load_store(); self.ticker.start()
    def _load_store(self) -> None:
        ...
        except Exception as e: log.warning("[status-pin] load store failed: %r", e)
    def _save_store(self) -> None:
        try:
            STORE_PATH.parent.mkdir(parents=True, exist_ok=True)
            data = {gid: rec.__dict__ for gid, rec in self._store.items()}
            STORE_PATH.write_text(json.dumps(data, ensure_ascii=False, indent=2), encoding="utf-8")
        except Exception as e: log.warning("[status-pin] save store failed: %r", e)
    def _build_embed(self) -> discord.Embed:
        presence = "presence=online"
        up = int(time.time() - self._started_at); m, s = divmod(up, 60); h, m = divmod(m, 60)
        ...
        emb.set_footer(text=PIN_TAG); return emb
    ...
    async def _ensure_message(self, guild: discord.Guild):
        ...
        if rec:
            ...
            if isinstance(ch, discord.TextChannel):
                try: return await ch.fetch_message(rec.message_id)
                except Exception: pass
        ...
        if msg:
            ...
            self._store[str(guild.id)] = StatusRecord(channel_id=ch.id, message_id=msg.id); self._save_store()
            return msg
        return None
    async def _update_message(self, msg: discord.Message) -> None:
        ...
        except Exception as e: log.warning("[status-pin] edit failed: %r", e)
    ...
    @ticker.before_loop
    async def before_ticker(self): await self.bot.wait_until_ready()
async def setup(bot: commands.Bot): await bot.add_cog(StatusPinUpdater(bot))
