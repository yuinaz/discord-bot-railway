...
def _render_db(phashes, dhashes=None, tiles=None):
    # Clean, compact JSON (no stray "\\n"), stable ordering to avoid noisy edits
...
class PhishHashInbox(commands.Cog):
    ...
    async def on_message_inbox(self, message: discord.Message):
        try:
            ...
            for h in all_p:
                if h not in sp: existing_p.append(h); sp.add(h); added_p.append(h)
            for h in all_d:
                if h not in sd: existing_d.append(h); sd.add(h); added_d.append(h)
            for t in all_t:
                if t not in st: existing_t.append(t); st.add(t); added_t.append(t)
            ...
            if parent and parent != ch:
                try:
                    e2 = discord.Embed(title="pHash update", colour=0xFF8C00)
