    ...
    async def _process_guild(self, guild: discord.Guild):
        ...
        async for m in target_thread.history(limit=LIMIT_MSGS, oldest_first=True):
            ...
            for att in m.attachments:
                ...
                if not name.endswith((".png",".jpg",".jpeg",".webp",".gif",".bmp",".tif",".tiff",".heic",".heif")):
                    continue
                raw = await att.read()
                if not raw: continue
                scanned_atts += 1
        ...
        for h in all_p:
            if h not in sp: existing_p.append(h); sp.add(h)
        for h in all_d:
            if h not in sd: existing_d.append(h); sd.add(h)
        for t in all_t:
            if t not in st: existing_t.append(t); st.add(t)
