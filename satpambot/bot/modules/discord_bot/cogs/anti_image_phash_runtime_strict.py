...
File ini TIDAK menghapus file/konfigurasi lain
...
cukup tambahkan sebagai cog baru.
...
async def _calc_phash_from_bytes(data: bytes) -> Optional[str]:
    ...
    Coba hitung pHash. Prioritas pakai imagehash
    ...
    fallback ke average hash buatan.
...
class AntiImagePhashRuntimeStrict(commands.Cog):
    ...
    def _is_allowed_attachment(att: discord.Attachment) -> bool:
        ...
        return any(name.endswith(ext) for ext in ALLOWED_EXTS) or any(ctype.startswith(p) for p in ALLOWED_MIME_PREFIX)
    ...
    async def _load_reference_from_channel(self, guild: discord.Guild) -> int:
        ...
        try:
            ...
            ref_ch: Optional[discord.TextChannel] = discord.utils.get(guild.text_channels, name=REF_CHANNEL_NAME)
    ...
    async def on_ready(self):
        ...
        LOG.info(
            ...
            "[phash-runtime.strict] 🔐 Runtime pHash DB loaded: %d entries (thresh=%d, preview=%s)",
