...
INTERVAL_SECONDS = 30  # fallback default interval (seconds)
...
PUSH_URL = os.getenv("METRICS_PUSH_URL") or os.getenv(
...
)  # e.g. https://your-app.onrender.com/api/metrics-ingest
                ...
                try:
                    ...
                    if getattr(g, "members", None):
                        ...
                        online += sum(
                            ...
                            if getattr(getattr(m, "status", None), "value", str(getattr(m, "status", "")))
    ...
    async def _push_http(self, payload: dict) -> None:
        ...
        def _post():
            ...
            req = urllib.request.Request(
                ...
                PUSH_URL, data=json.dumps(payload).encode("utf-8"), headers=headers, method="POST"
