...
async def _post_ingest(payload: dict) -> None:
    ...
    try:
        ...
        async with aiohttp.ClientSession() as sess:
            ...
            await sess.post(INGEST_URL, json=payload, headers={"X-INGEST-TOKEN": INGEST_TOKEN}, timeout=15)
...
async def notify_bot_active(guild: discord.Guild, bot_name: str):
    ...
    if log_channel:
        ...
        try:
            ...
            await log_channel.send(f"🟢 Bot aktif sebagai **{bot_name}** pada {datetime.now():%Y-%m-%d %H:%M:%S}")
...
async def notify_to_ngobrol(guild: discord.Guild, user: discord.User, reason: str):
    ...
    if ngobrol_ch:
        ...
        try:
            ...
            sticker = None
            if 'sticker' in locals() and sticker:
