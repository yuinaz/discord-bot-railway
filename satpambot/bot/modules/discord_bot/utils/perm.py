...
NAMES = {
    ...
    p.strip().lower() for p in (os.getenv("MOD_ROLE_NAMES", DEFAULT_NAMES).replace(";", ",")).split(",") if p.strip()
...
def require_mod():
    ...
    async def pred(ctx: commands.Context) -> bool:
        ...
        if not ctx.guild:
            ...
            raise commands.CheckFailure("Gunakan perintah ini di dalam server.")
        ...
        if not m:
            ...
            raise commands.CheckFailure("Tidak bisa mengambil data member.")
        ...
        if not is_mod(m):
            ...
            raise commands.CheckFailure("Kamu tidak punya izin untuk menjalankan perintah ini.")
