...
try:
    ...
    from PIL import Image, ImageFile, ImageFilter, ImageOps, ImageSequence
...
def _augment_variants(img: "Image.Image", max_extra: int = 4):
    ...
    try:
        ...
        if count < max_extra:
            ...
            yield img.rotate(7, expand=True, fillcolor=(0, 0, 0))
    ...
    try:
        ...
        if count < max_extra:
            ...
            if w - 2 * dw > 8 and h - 2 * dh > 8:
                ...
                yield img.crop((dw, dh, w - dw, h - dh))
...
def phash_list_from_bytes(



    data: bytes, max_frames: int = 6, augment: bool = False, augment_per_frame: int = 4
...
def tile_phash_list_from_bytes(
...
def orb_descriptors_from_bytes(
...
def phash_hex(img):
    ...
    """Compat shim: return perceptual hash hex string."""
