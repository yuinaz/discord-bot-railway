...
MOD_ROLE_IDS = {int(x) for x in os.getenv("MOD_ROLE_IDS", "").replace(" ", "").split(",") if x.isdigit()}



MOD_ROLE_NAMES = [s.strip().lower() for s in os.getenv("MOD_ROLE_NAMES", "moderator,admin,mod").split(",") if s.strip()]
