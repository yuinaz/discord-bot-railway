...
async def _find_channel_by_id_or_name(bot: discord.Client, guild: discord.Guild) -> Optional[discord.TextChannel]:
    ...
    names = [
        ...
        os.getenv("STATUS_CHANNEL_NAME", ""),



        os.getenv("LOG_CHANNEL_NAME", ""),
...
def log_startup_status(bot: discord.Client, guild: discord.Guild) -> None:
    ...
    log.info(
        ...
        "[status] LOG_CHANNEL_ID_RAW='%s' parsed=%s LOG_CHANNEL_NAME='%s'",



        raw_log_id or raw_status_id,
    ...
    if isinstance(ch, discord.TextChannel):
        ...
        log.info(
            ...
            "[status] using log channel: #%s (id=%s) in guild='%s' (id=%s)",
        ...
        log.warning(
            ...
            "[status] log channel not found in guild='%s' (id=%s)",
        ...
        for m in pins or []:
            ...
            if getattr(m, "author", None) and getattr(bot, "user", None) and m.author.id == bot.user.id:
                ...
                body = (
                    ...
                    + " ".join([(e.footer.text or "") for e in (m.embeds or []) if getattr(e, "footer", None)])
