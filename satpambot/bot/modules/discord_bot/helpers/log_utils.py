...
async def _find_channel_by_id_or_name(bot: discord.Client, guild: discord.Guild) -> Optional[discord.TextChannel]:
    ...
    for ch in getattr(guild, "text_channels", []):
        try:
            if getattr(ch, "name", None) == target_name:
...
async def upsert_status_embed_in_channel(bot: discord.Client, ch: discord.abc.Messageable):
    async with _lock:
        try:
            ...
            if guild is None and getattr(bot, "guilds", None):
            ...
            try:
                sticky_store.upsert_guild(guild.id,
                                          channel_id=getattr(ch, "id", None),
                                          message_id=getattr(msg, "id", None),
                                          last_edit_ts=int(datetime.utcnow().timestamp()))
            except Exception:
                pass
        except discord.Forbidden:
            log.warning("[status] tidak punya izin untuk kirim/edit di #%s", getattr(ch, "name", ch))
