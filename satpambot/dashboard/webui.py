...
# ===== ADD-ONLY: pHash meta & after-request logger (no config changes) =====
# - Does NOT modify existing endpoints
# - Adds a new meta endpoint for dashboard: /dashboard/api/phish/phash/meta
# - Adds after_app_request hooks (bp + api_bp) to log autoban & count for /phish/phash

def _phash_security_cfg():
    # Minimal reader for autoban setting (optional file).
    # Reads DATA_DIR()/phish_lab/security_settings.json; default autoban=True if missing.
    try:
        import json, os
        from pathlib import Path as _P
        base = DATA_DIR()
        p = (_P(base) / "phish_lab" / "security_settings.json")
        if p.exists():
            j = json.loads(p.read_text(encoding="utf-8") or "{}")
            return bool(j.get("autoban", True))
    except Exception:
        pass
    return True

@bp.get("/api/phish/phash/meta")
def _phash_meta_dashboard():
    try:
        hashes = _phash_blocklist_read()
    except Exception:
        hashes = []
    try:
...
# After-request logger: dashboard blueprint
...
def _after_log_phash_bp(resp):
    try:
        ...
        if p.endswith("/phish/phash"):
            try:
                # Count from file (not from response) to be robust
                cnt = len(_phash_blocklist_read())
            except Exception:
                cnt = None
            try:
                autoban = _phash_security_cfg()
            ...
            current_app.logger.info(
                "[phash] autoban=%s count=%s referer=%s ua=%s",
                autoban, cnt, request.referrer, request.headers.get("User-Agent","")
            )
    except Exception:
        pass
    return resp

# After-request logger: public /api blueprint
