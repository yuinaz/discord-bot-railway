...
need = [".login-card{", ".login-form input[type=text]", ".login-btn", ".particles-js-canvas-el{opacity:.35"]
base_css = """
...
if base_html.exists():
    ...
    if "dashboard.css" not in b:
        b = b.replace("</head>", "  <link rel=\"stylesheet\" href=\"{{ url_for('static', filename='dashboard.css') }}\">\n</head>")
...
if "from flask import" in s:
    s = re.sub(
        ...
        lambda m: "from flask import " + ", ".join(sorted(set([x.strip() for x in (m.group(1)+", request, jsonify, redirect, url_for").split(",") if x.strip()]))),
        s, count=1
    )
else:
