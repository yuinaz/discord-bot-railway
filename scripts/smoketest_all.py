...
def check_syntax() -> Tuple[bool, List[Dict]]:
    ...
    if not fails:
        ...
        print(f"OK  : Python syntax check ({count} files)")
...
def check_json() -> Tuple[bool, List[Tuple[str, str]]]:
    ...
    for rel in JSON_PATHS:
        ...
        try:
            ...
            errs.append((rel, f"INVALID JSON: {e}"))
    ...
    if not errs:
        ...
        print("OK  : All JSON files valid/exist")
...
def check_upsert_calls() -> Tuple[bool, List[str]]:
    ...
    for pth in ROOT.rglob("*.py"):
        ...
        if re.search(r"await\s+upsert_status_embed_in_channel\(\s*log_ch\s*,\s*['\"]", txt):
        ...
        if re.search(r"await\s+upsert_status_embed_in_channel\(\s*ch\s*,\s*['\"]", txt):
    ...
    if bad:
        ...
        for f in bad:
            ...
            print(f"FAIL: {f} :: legacy upsert call detected")
        ...
        print("OK  : All upsert call-sites use bot,ch pattern")
...
def check_features() -> Tuple[bool, List[str]]:
    ...
    if login.exists():
        ...
        if not any(x in t for x in ["tsparticles", "particles"]):



            warns.append("login.html: particles background not found")
    ...
    if sec.exists():
        ...
        if not any(x in t for x in ["dropzone", "ondrop", "dragover", "data-dropzone"]):



            warns.append("security.html: dropzone not found")
    ...
    if sc.exists() and "/healthz" not in _read(sc):



        warns.append("sitecustomize.py: /healthz not exposed")
    ...
    if warns:
        ...
        print("OK  : Feature markers present")
...
def check_http_test_client() -> Tuple[bool, List[str]]:
    ...
    try:
        ...
        if r.status_code in (301, 302) and (
            ...
            r.headers.get("Location", "").endswith("/dashboard") or "/dashboard" in r.headers.get("Location", "")
            ...
            print("FAIL: / expected redirect -> /dashboard")
        ...
        if r.status_code == 200:
            ...
            print("OK  : GET /dashboard/login :: 200")
            ...
            print("FAIL: GET /dashboard/login :: %d" % r.status_code)



            fails.append("/dashboard/login")



        if r.status_code == 200 and b"lg-card" in r.data:



            print("OK  : login layout present (lg-card)")
        ...
        print(f"OK  : HEAD /healthz :: {r.status_code}")
        ...
        print(f"OK  : HEAD /uptime :: {r.status_code}")
        ...
        if r.status_code == 200:
            ...
            print("OK  : GET /api/ui-config :: 200")
        ...
        if r.status_code == 200:
            ...
            if "gtake" in html:
                ...
                print("OK  : dashboard layout = gtake")



            if "canvas" in html or "requestanimationframe" in html:



                print("OK  : dashboard has 60fps canvas")



            if "dropzone" in html or "ondrop" in html:



                print("OK  : dashboard has dropzone")
        ...
        if r.status_code == 200:
            ...
            print("OK  : GET /api/phish/phash :: 200")
        ...
        print(f"OK  : GET /logout :: {r.status_code}")
        ...
        if r.status_code in (301, 302):
            ...
            print(f"OK  : GET /dashboard/logout -> redirect :: {r.status_code} -> {r.headers.get('Location')}")
...
def main():
    ...
    sections.append(("syntax", ok1))



    ok2, _ = check_required()



    sections.append(("required", ok2))
    ...
    sections.append(("json", ok3))



    ok4, _ = check_upsert_calls()



    sections.append(("upsert", ok4))



    ok5, _ = check_features()



    sections.append(("features", ok5))



    ok6, _ = check_http_test_client()



    sections.append(("http_test_client", ok6))
