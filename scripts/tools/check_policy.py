...
def validate_and_normalize(data: Dict[str, Any]):
    ...
    out["allowlist_guild_ids"] = [



        int(x) for x in data.get("allowlist_guild_ids", out["allowlist_guild_ids"]) if str(x).isdigit()
    ...
    out["allowlist_invite_codes"] = [



        str(x).strip() for x in data.get("allowlist_invite_codes", out["allowlist_invite_codes"]) if str(x).strip()
    ...
    out["nsfw_invite_keywords"] = [



        str(x).lower() for x in data.get("nsfw_invite_keywords", out["nsfw_invite_keywords"]) if str(x).strip()
