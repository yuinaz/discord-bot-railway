...
def kind(p: Path):
    ...
    if "/templates/" in s and s.endswith(".html"):
    ...
    if "/static/" in s and any(s.endswith(ext) for ext in [".js", ".css", ".svg", ".png", ".jpg", ".jpeg", ".webp"]):
...
def score(p: Path, k: str):
    ...
    if k == "template":
        ...
        if "/templates/dashboard/" not in s:
        ...
        try:
            ...
            if re.search(r'{%\s*extends\s+"base\.html"\s*%}', t):
        ...
        if any(x in s for x in ["app.py", "main.py"]):
...
files = [
    ...
    if p.is_file() and "_reports" not in str(p) and "/unused/" not in str(p).replace("\\", "/")
...
for name, lst in by_name.items():
    ...
    best = max(lst, key=lambda p: score(p, kinds[p]))  # noqa: E731
