...
"""
import os, subprocess, sys, json, base64, urllib.request, urllib.error
...
def commit_file(path: str, content: str, message: str):
    ...
    if sha:
        payload["sha"] = sha
    req = urllib.request.Request(api, data=json.dumps(payload).encode("utf-8"), method="PUT", headers={
        "Authorization": f"Bearer {token}",
        "Accept": "application/vnd.github+json",
        "User-Agent": "satpambot-freezer"
    })
